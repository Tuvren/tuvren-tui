...
- **Description:** Implement ABI 2.0 decoding, opcode/property/value compatibility, typed complex payloads with no raw post-validation record slice, typed grapheme-range properties, transaction-local node references, caller-owned mappings and indexed mutation results, bounded query/copy-out reads, exact-version loading, panic containment, the exact public `TuvrenStatus` return/result initialization matrix and no-command sentinel, generated production symbol coverage, TypeScript/Rust byte-fixture parity, and one Render Pass request per committed batch. The validated model preserves request-render, create initial generation, Collection indexes, Transcript flags, distinct animation cancel versus value-bearing operations, complete animation policy/values, discriminated UTF-8 announcement versus opaque clipboard content, terminal target/timeout, and diagnostic flags, and resolves nested layout/style/semantic/span/validation/Collection/identity/Transcript offsets into typed records before mutation.
- **Acceptance:**
  - **Mode:** invariant
  - **Evidence:**

```text
Generated checks cover every implemented ABI symbol, fixed field, and nested record; Rust and TypeScript decode checked-in success, buffer-too-small, invalid, stale, unavailable, and panic-contained status/output fixtures identically; grapheme selection, request-render scheduling, create generation, canonical Grid tracks/percentage points, ordered Collection operations, Transcript flags, value-bearing versus cancel animation, clipboard bytes versus UTF-8 announcement, terminal target/timeout, and diagnostic capture flags survive validation; nested-offset fixtures produce typed resolved layout/style/semantic/span/validation/Collection/identity/Transcript values with no transaction-relative record slice reaching mutation; return and result statuses agree; failure outputs are fully initialized; query and transaction outputs preflight caller capacities and never expose retained pointers; malformed, misaligned, overlapping, stale, oversized, or incompatible records are rejected before mutation; post-validation unexpected failure freezes and discards the context.
```
