...
- **Description:** Implement and run the Stage 3 aggregate command over contract, capability-map, semantic, terminal, every named bounded fuzz target, bundle, envelope, comparative, devtools, adoption, OpenCode, supply-chain, target, package, schema, migration, restoration, error-registry and snapshot/trace/replay/benchmark cross-field validation, and atomic-manifest evidence. Seeded contract cases include Semantic Tree graph corruption, Trace ordering/correlation/privacy/payload corruption, all Command completion variants, document-bound TextArea authority, and independent Collection scroll-position observation.
- **Acceptance:**
  - **Mode:** runbook_probe
  - **Evidence:**
