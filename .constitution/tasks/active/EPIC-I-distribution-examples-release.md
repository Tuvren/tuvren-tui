...
- **Scope (In-Scope Files):** root `package.json`, `scripts/check-release-candidate.ts`, release validation scripts, schema-valid evidence, release-candidate report and migration guide
...
- **Expected Success Output:** every P0 command including fresh `check:abi-parity` and `test:host`, registered error, required benchmark check, trace payload, replay expectation, evidence row, and named cross-field validator passes and `validateAtomicReleaseManifest` accepts exactly six matching artifacts
...
- **Description:** Implement and run the Stage 3 aggregate command over exact toolchain, contract, fresh ABI parity, the canonical strict host aggregate (FFI, JSX, Commands, Effect, animation ordering, examples, install, runner), capability-map, semantic, terminal, every named bounded fuzz target, bundle, envelope, comparative, devtools, adoption, OpenCode, supply-chain, target, package, schema, migration, restoration, error-registry and snapshot/trace/replay/benchmark cross-field validation, and atomic-manifest evidence. Seeded contract cases include Semantic Tree graph/scalar and Issue tuple/interval corruption, Trace ordering/identity-equality/instance-subject mapping/parent/domain/status/start/privacy/batch/wrap-boundary/foreign-or-misbased-or-mis-inherited-snapshot corruption, runtime replay handler suppression and exactly-once transaction application, surrogate key and wheel-wire drift, all four bound Command failure/environment propagation paths, all Command completion variants, Effect/imperative Animation Event drain ordering, document-bound TextArea authority, controlled/local List/Table/Select/Menu/palette selection, Transcript controller teardown/rebind isolation, and executor-cached Collection/Transcript observations.
- **Acceptance:**
  - **Mode:** runbook_probe
  - **Evidence:**

```text
A fresh `check:release-candidate` run executes `check:toolchain`, `check:abi-parity`, and `test:host` against the candidate revision, produces one human-readable candidate report and the schema-valid atomic manifest, skips no required subcommand, and proves all artifact versions and source revisions match with no alpha artifact represented as final.
```
