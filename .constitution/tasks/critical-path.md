...
- The audit report is a point-in-time artifact written against the pre-v9 backlog; where it suggests ticket homes, map them to the current plan: addendum A1 (loop unification + wake seam, formerly "Epic U" scope) → `ARCH-W001`; addendum A2 (encoder hoist / delta layer / indexed mutation) → `PERF-V008`/`PERF-V009`/`PERF-V010`; addendum A3 and its "Epic U SDK-U005" placement (palette wiring + `subscribe()`) → `ARCH-W006`; direction suggestion 1 (streaming-scale epic) → Epic V; suggestion 2 (OpenCode client) → Epic Y; suggestion 3 (four orphaned bindings) → `SDK-X002`.
