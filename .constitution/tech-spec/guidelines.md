...
**v9.0.23** — corresponds to `.constitution/tech-spec/changelog.md`.
...
- Normalized keyboard, pointer-button, pointer-move, and wheel Events share the closed `EventModifier` set (`shift`, `control`, `alt`, `super`) used by replay and ABI codecs. `TUVREN_EVENT_MOD_MASK` is the complete wire mask; every live, test, and replay decoder rejects unknown bits before dispatch instead of ignoring or normalizing them.
...
- Collection and Transcript `lastVisibleRange()` follow the same executor-populated cache rule: they return `undefined` before the first committed observation, include transaction/render identities, and never call the ABI synchronously. A controller returned by `useTranscriptController()` is unbound until supplied as one Transcript's `controller` prop and cannot bind to two live Transcripts. Its operation methods return `unbound` without enqueueing or retaining work until binding and `accepted` only after enqueueing to its current target. Teardown cancels still-pending operations, clears every cached observation, and unbinds before the controller may target another Transcript; bind → observe → teardown → rebind fixtures prove no old cache or queued work crosses targets. Imperative Transcript constructors omit the declarative controller prop and expose only their own controller identity, which follows the same teardown disposition.
...
- Validation converts each transaction command to its typed `ValidatedPayload` and preserves every behavior-bearing header/fixed-record field, including render request, initial generation, grapheme ranges, Collection indexes, Transcript flags, complete animation values/policy, terminal target/timeout, and diagnostic flags. It resolves every nested offset into typed dimensions, responsive layouts, style/token/rule values, semantic scalars and relationships, StyledText spans with optional style, validation rules, Collection items/identities, and Transcript blocks. Animation cancellation is a distinct identity-only payload. Terminal requests discriminate validated UTF-8 announcements from opaque clipboard bytes. Runtime mutation never reparses or receives a raw transaction-relative record slice; borrowing remains only for resolved UTF-8 strings and explicitly opaque terminal content. Contract checks compare C fields and nested records to Rust validated-model ownership so a successful decode cannot silently discard semantics.
...
- Dimension and responsive percentages use percentage points: `"50%"` and `minWidthPercent: 50` both encode `50.0`, never `0.5`. Percentages are finite in [0, 100], cell thresholds are finite and nonnegative, negative zero normalizes to positive zero, absent float fields are zero, and paired minimums cannot exceed maximums. Grid-track kinds use the one canonical field allocation documented in `native-abi.h`; every unused field is zero.
...
- Public Color strings accept only ASCII `#RRGGBB` and `#RRGGBBAA`; object channels must be finite integers from 0 through 255 and omitted alpha equals 255. Validation rejects every other spelling/value without rounding or clamping. The host packs both forms as `0xRRGGBBAA`, with red in bits 31–24 and alpha in bits 7–0, for style, Theme, and animation records.
...
- ABI-local status values are fixed to success, buffer-too-small, invalid-input, stale-context, unavailable, and panic-contained and never escape the private bridge as SDK errors. Every `int32_t` export except `tui_abi_version` returns exactly a nonnegative `TuvrenStatus`; Apply/Query/Drain result status agrees with it. The per-status matrix fully initializes outputs, forbids partial writes, zeros meaningless fields, and uses `TUVREN_NO_COMMAND_INDEX` unless one decoded command caused invalid input. TypeScript/Rust byte fixtures cover every return/result combination before mapping details to public errors.
