...
**v9.0.11** — corresponds to `.constitution/tech-spec/changelog.md`.
...
- Collection controllers and observers expose native-owned, generation-stamped scroll position independently of visible range, including a stable key anchor, signed row offset, and optional pixel offset.
...
- Raw input, clipboard content, terminal payloads, environment values, and absolute paths are redacted by default. Confirmed full-content traces include the exact versioned Event and committed-mutation bytes required for runtime replay while environment values and absolute paths remain redacted.
...
- `validateDiagnosticTraceRecords` applies the exact kind-selected payload contracts from `trace-validation.json`, rejects unknown fields, checks registered errors, proves unsigned sequence uniqueness/order, timestamp order, required nonempty correlations and backward-only causal references, and refuses runtime replay unless a confirmed full-content Trace carries the exact versioned Event, transaction, and mutation bytes with available migrations.
