...
- v8.2.0 - Executed Epic T: `ExtensionRegistry` with activation/deactivation lifecycle, auto-tracking disposal, and failure isolation; six contribution slots (commands, keymaps, palette, devtools, themes, examples) wired into `ExtensionContext`; `CommandPalette` integration for palette title overrides; contribution validation for all slot types; pre-GA compatibility markers; `plugin-demo.ts` example; 48 focused tests in `ts/test-extensions.test.ts`; 501 of 508 host tests pass (6 pre-existing failures + bundle budget note).
...
- **Total Active Story Points:** 56 (Epic R, Epic S, and Epic T shipped)
...
- No `v1.0` compatibility guarantee in Epics U-V; plugin slots and Effect integration remain pre-GA.
...
- Epic T (Plugin Slots and Extensibility) shipped as pre-GA contribution points: `ExtensionRegistry` with activation/deactivation lifecycle, auto-tracking disposal, and failure isolation; six contribution slots (commands, keymaps, palette, devtools, themes, examples) wired into `ExtensionContext`; `CommandPalette` integration for palette title overrides; contribution validation for all slot types; pre-GA JSDoc markers; `plugin-demo.ts` example; 48 focused tests in `ts/test-extensions.test.ts`.
...
Epic T is archived as a shipped pre-GA extensibility wave. It added `ExtensionRegistry` with activation/deactivation lifecycle, auto-tracking disposal via lightweight proxy wrappers, and per-extension failure isolation in `ts/src/extensions.ts`. Six bounded contribution slots (`commands`, `keymaps`, `palette`, `devtools`, `themes`, `examples`) are exposed through `ExtensionContext`. `CommandPalette` integrates with a palette contribution registry for title overrides. Contribution validation rejects empty IDs and titles before registration. Pre-GA JSDoc markers document the breaking-change posture on all public plugin types. `examples/plugin-demo.ts` exercises all contribution slots plus activation, deactivation, and diagnostics. 48 focused tests in `ts/test-extensions.test.ts` cover registration, lifecycle, all six slot types, failure isolation, validation, diagnostics, and CommandPalette integration. All extension APIs remain pre-GA per ADR-T46; stable compatibility guarantees are deferred to `v1.0`.
...
### 6.6 Archived v7 Docs-Maintenance Wave
