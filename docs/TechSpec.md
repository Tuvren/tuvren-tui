...
- **Decision:** Execute a hard pre-`1.0` rename of the public surface from Kraken to Tuvren. The approved target-state uses `tuvren-tui` as the public package, `Tuvren` as the primary host facade, `TUVREN_LIB_PATH` as the resolver override, `tuvren_tui` / `libtuvren_tui.*` for the native crate and shared library names, and `tuvren-tui-*` release artifact names. The C ABI prefix remains `tui_*` to avoid gratuitous ABI churn.
...
- **Decision:** Treat commands and keymaps as the first sanctioned framework-level host services after the productization wave. They live in the Host Layer over the existing imperative command protocol and native event stream. Plugin slots are explicitly deferred until the commands/keymap contract proves stable, and they are not part of the active implementation contract in this document yet.
