...
### Resolver Contract
- Preserve the Brownfield resolver contract unless the changes are part of the Epic P productization migration defined in `.constitution/tasks/completed/EPIC-P-tuvren-identity.md`; for that wave, follow the approved target-state contract in `.constitution/tech-spec/stack.md` §5 (Release and Distribution Matrix).
...
- Repo-side FFI tests and benchmark harnesses that `dlopen` directly must bypass old packaged artifacts and validate the local Cargo-built branch artifact.
